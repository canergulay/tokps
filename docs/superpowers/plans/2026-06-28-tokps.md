...
- Module path: **`tokps`**. Internal imports use `tokps/internal/...`.
...
Run:
```bash
cd /Users/canergulay/code/indie/tokps
go mod init tokps
```
Expected: creates `go.mod` containing `module tokps` and a `go 1.23` line.
...
Run: `go test ./internal/sse/`
Expected: PASS (`ok  	tokps/internal/sse`).
...
- Produces: the `tokps` binary entrypoint. (No unit test — thin wiring verified by `go build` and `go vet`; logic lives in tested packages.)
